...
class MirrorToolkit:
    def __init__(self, student_id: str, history: Dict, ltm_data: Dict = None,
        ...
        self.setting = setting  # S1/S2/S3 for vectorstore cache isolation
    ...
    def _load_or_build_vectorstore(self):
        ...
        # Setting-aware cache path to prevent data leakage across S1/S2/S3
        cache_suffix = f"_{self.setting}" if self.setting != 'S1' else ""
        persist_dir = os.path.join(Config.CHROMA_DIR, f"mirror_{self.student_id}{cache_suffix}")
        collection_name = f"mirror_{self.student_id}{cache_suffix}"
